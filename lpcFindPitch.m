...
for i = 1:n
    ...
    % Use a temporary variable for the result of findPeakFreqs
    temp_f = findPeakFreqs(xi, 1, np);
    
    % Ensure that temp_f is a column vector
    temp_f = temp_f(:);
    
    % Assign temp_f to the corresponding column in f
    f(1:length(temp_f), i) = temp_f;
